...
// ============================================================================
// GoHighLevel CRM Helpers (maps overlay)
//
// This is a content script injected into google.com/maps, so page-origin CORS
// blocks direct backend fetches. Every GHL call is routed through the
// background service worker via chrome.runtime.sendMessage (the worker holds the
// session token and builds the GHL payload from the raw scraped item).
// ============================================================================
function ghlSend_maps(msg, cb) {
    try {
...
function ghlIsConnected_maps(cb) {
    ghlSend_maps({ type: 'GHL_IS_CONNECTED' }, function (resp) {
        cb(Boolean(resp && resp.connected));
    });
}

function ghlFetchLocations_maps(force, cb) {
    ghlSend_maps({ type: 'GHL_FETCH_LOCATIONS', force: !!force }, function (resp) {
        cb(resp && Array.isArray(resp.items) ? resp.items : []);
    });
}

function ghlFetchTags_maps(locationId, force, cb) {
    if (!locationId) { cb([]); return; }
    ghlSend_maps({ type: 'GHL_FETCH_TAGS', locationId: locationId, force: !!force }, function (resp) {
        cb(resp && Array.isArray(resp.items) ? resp.items : []);
    });
}

function ghlFetchUsers_maps(locationId, force, cb) {
    if (!locationId) { cb([]); return; }
    ghlSend_maps({ type: 'GHL_FETCH_USERS', locationId: locationId, force: !!force }, function (resp) {
        cb(resp && Array.isArray(resp.items) ? resp.items : []);
    });
}

function ghlFetchPipelines_maps(locationId, force, cb) {
    if (!locationId) { cb([]); return; }
    ghlSend_maps({ type: 'GHL_FETCH_PIPELINES', locationId: locationId, force: !!force }, function (resp) {
        cb(resp && Array.isArray(resp.items) ? resp.items : []);
    });
}

function ghlCheckDuplicate_maps(locationId, phones, email, cb) {
    if (!locationId) { cb({ exists: false }); return; }
    ghlSend_maps({ type: 'GHL_CHECK_DUPLICATE', locationId: locationId, phones: phones || [], email: email || '' }, function (resp) {
        cb(resp || { exists: false });
    });
}

function ghlSendLead_maps(locationId, item, opts, cb) {
    ghlSend_maps({ type: 'GHL_SEND_LEAD', locationId: locationId, item: item, opts: opts }, function (resp) {
        if (!resp) { cb({ success: false, error: 'Not connected to GoHighLevel. Connect in the extension popup.' }); return; }
        cb(resp);
    });
}

// ============================================================================
// GoHighLevel Confirmation Modal (maps overlay) — mirrors the results-tab modal
// ============================================================================
function showCrmConfirmation_maps(item, onConfirm) {
    ...
    modal.innerHTML =
        ...
        '<tr><td style="padding:5px 8px 5px 0;font-weight:700;color:#80868b;white-space:nowrap;vertical-align:top;">Sub-Account</td><td style="padding:5px 0;color:#202124;">' +
        ...
        '<tr><td style="padding:5px 8px 5px 0;font-weight:700;color:#80868b;white-space:nowrap;vertical-align:top;">Tag</td><td style="padding:5px 0;color:#202124;">' +
        ...
        '<div style="font-size:11px;color:#80868b;margin-top:4px;">Picking a user routes the lead straight to them.</div></td></tr>' +
        '<tr><td style="padding:5px 8px 5px 0;font-weight:700;color:#80868b;white-space:nowrap;vertical-align:top;">Opportunity</td><td style="padding:5px 0;color:#202124;">' +
        ...
        '<tr id="gmes-confirm-pipeline-row"><td style="padding:5px 8px 5px 0;font-weight:700;color:#80868b;white-space:nowrap;vertical-align:top;">Pipeline</td><td style="padding:5px 0;color:#202124;">' +
        ...
        '<tr id="gmes-confirm-stage-row"><td style="padding:5px 8px 5px 0;font-weight:700;color:#80868b;white-space:nowrap;vertical-align:top;">Stage</td><td style="padding:5px 0;color:#202124;">' +
        ...
        '<tr><td style="padding:5px 8px 5px 0;font-weight:700;color:#80868b;white-space:nowrap;vertical-align:top;">Note</td><td style="padding:5px 0;color:#202124;">' +
        '<textarea id="gmes-confirm-note" style="width:100%;min-height:58px;padding:6px 10px;border:1.5px solid #e2e5eb;border-radius:7px;font-family:inherit;font-size:12.5px;background:#fff;color:#202124;resize:vertical;">' + esc(item.note || '') + '</textarea></td></tr>' +
        '</table></div>' +
    ...
    function populateForLocation(locationId, defaults) {
        ...
        ghlFetchTags_maps(locationId, false, function (tags) {
            var preselect = item.ghlTag || defaults.tag || '';
        ...
        ghlFetchUsers_maps(locationId, false, function (users) {
        ...
        ghlFetchPipelines_maps(locationId, false, function (pipelines) {
    ...
    chrome.storage.local.get([
        ...
        var activeLoc = (item.ghl && item.ghl.locationId) || data.gmes_ghl_default_location || '';
        var defaults = {
        ...
        ghlFetchLocations_maps(false, function (locs) {
...
// ============================================================================
// End of GoHighLevel CRM Helpers
// ============================================================================
...
(function () {
    ...
    chrome.storage.onChanged.addListener((changes, area) => {
        ...
        var extOn = changes.gmes_extension_enabled
            ? changes.gmes_extension_enabled.newValue === true
            : null;
    ...
    function scrapeCurrentPlace() {
        ...
        // Coordinates from the place URL (!3d<lat>!4d<lng>, @<lat>,<lng> fallback).
        var coordMatch = (window.location.href || '').match(/!3d(-?\d+\.\d+)!4d(-?\d+\.\d+)/) ||
            (window.location.href || '').match(/@(-?\d+\.\d+),(-?\d+\.\d+)/);
        if (coordMatch) { item.lat = coordMatch[1]; item.lng = coordMatch[2]; }
        ...
        try {
            ...
            if (!item.businessTimings) {
                ...
                if (m) item.businessTimings = m[0].replace(/\s+/g, ' ').trim();
            }
        } catch (e) {}

        // 8. City / State / ZIP — parse from the full formal address
        // ("123 Main St, Columbus, OH 43215, United States"); fall back to the
        // page title when no address is available.
        const loc = parseCityState(item.address);
        item.city = loc.city;
        item.state = loc.state;
        item.zip = loc.zip;
        if (!item.city) {
            const cityMatch = (document.title || '').match(/,\s*([^,]+)(?:,[^,]+)?\s*-\s*Google Maps/);
    ...
    function createOverlay() {
      ...
      <div class="gmes-content">
        ...
        <div class="gmes-field">
          ...
          <div class="gmes-field-value">${escapeHtml(item.industry) || '<em style="color:#9aa0a6">N/A</em>'}</div>
        </div>
        <div class="gmes-field">
          <div class="gmes-field-label">
    ...
    `;
        ...
        // Check GHL connection/status and wire the Send to GoHighLevel button
        ...
        if (crmStatusDiv && crmBtn) {
            crmBtn.innerHTML = crmBtn.innerHTML.replace('Send to CRM', 'Send to GoHighLevel');
            ghlIsConnected_maps((connected) => {
                if (!connected) {
                    crmBtn.style.display = 'flex';
                    crmBtn.disabled = true;
                    crmBtn.style.opacity = '0.55';
                    crmBtn.style.cursor = 'not-allowed';
                    crmBtn.title = 'Connect GoHighLevel in the extension popup to enable';
                    crmStatusDiv.innerHTML = '\uD83D\uDD12 Connect in the extension popup to send leads. <a href="#" id="gmes-crm-login-link" style="color:#1a73e8;font-weight:700;text-decoration:underline;">Connect \u2192</a>';
                ...
                // Optional dedup pre-check against the default sub-account.
                chrome.storage.local.get(['gmes_ghl_default_location'], (data) => {
                    const dupLoc = data.gmes_ghl_default_location || '';
                    ...
                    crmBtn.addEventListener('click', () => {
                        const freshItem = scrapeCurrentPlace();
                        const noteEl = document.getElementById('gmes-note-input');
                        if (noteEl) freshItem.note = noteEl.value.trim();

                        // Show confirmation modal (GHL sub-account/tag/assignee/pipeline)
                        showCrmConfirmation_maps(freshItem, () => {
                            crmBtn.disabled = true;
                            crmBtn.textContent = 'Sending to GHL…';
                            const opts = freshItem.ghl || {};
                            ghlSendLead_maps(opts.locationId || dupLoc, freshItem, opts, (result) => {
                                if (result && result.success) {
                                    crmBtn.style.display = 'none';
                                    crmStatusDiv.textContent = result.duplicate ? '✓ In GHL (existing)' : '✓ Sent to GHL';
                                    crmStatusDiv.style.background = '#e8f5e9';
                                    crmStatusDiv.style.color = '#34a853';
                                    crmStatusDiv.style.display = 'block';
                                    // Auto-add to local list
                                    freshItem.crmSynced = true;
                                    freshItem.ghlContactId = result.contactId || '';
                                    freshItem.ghlOpportunityId = result.opportunityId || '';
                                    if (result.contactId) freshItem.crmExistingId = result.contactId;
                                    chrome.runtime.sendMessage({ type: 'MANUAL_ADD_ITEM', item: freshItem });
                                } else {
                                    crmBtn.disabled = false;
                                    crmBtn.textContent = 'Retry GHL Send';
                                    alert('GHL send failed: ' + ((result && result.error) || 'Unknown error'));
                                }
                            });
                        });
                    });
                    ...
                    if (!dupLoc) { showSendButton(); return; }
                    ghlCheckDuplicate_maps(dupLoc, checkPhones, item.email || '', (res) => {
                        if (res && res.exists) {
                            crmStatusDiv.textContent = '✓ Already in GHL';
                            crmStatusDiv.style.background = '#e8f5e9';
                            crmStatusDiv.style.color = '#34a853';
                            crmStatusDiv.style.display = 'block';
                        } else {
