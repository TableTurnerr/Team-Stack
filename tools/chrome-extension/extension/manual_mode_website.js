...
// ============================================================================
// GoHighLevel CRM Helpers (via background service worker)
// ============================================================================
// Content-script fetches are blocked by the page's origin CORS, so every GHL
// call is routed through the background worker over chrome.runtime.sendMessage.

function ghlIsConnected_website(cb) {
    chrome.runtime.sendMessage({ type: 'GHL_IS_CONNECTED' }, function (resp) {
        cb(Boolean(resp && resp.connected));
    });
}

function ghlFetchLocations_website(force, cb) {
    chrome.runtime.sendMessage({ type: 'GHL_FETCH_LOCATIONS', force: Boolean(force) }, function (resp) {
        cb((resp && resp.items) || []);
    });
}

function ghlFetchTags_website(locationId, force, cb) {
    chrome.runtime.sendMessage({ type: 'GHL_FETCH_TAGS', locationId: locationId, force: Boolean(force) }, function (resp) {
        cb((resp && resp.items) || []);
    });
}

function ghlFetchUsers_website(locationId, force, cb) {
    chrome.runtime.sendMessage({ type: 'GHL_FETCH_USERS', locationId: locationId, force: Boolean(force) }, function (resp) {
        cb((resp && resp.items) || []);
    });
}

function ghlFetchPipelines_website(locationId, force, cb) {
    chrome.runtime.sendMessage({ type: 'GHL_FETCH_PIPELINES', locationId: locationId, force: Boolean(force) }, function (resp) {
        cb((resp && resp.items) || []);
    });
}

function ghlCheckDuplicate_website(locationId, phones, email, cb) {
    chrome.runtime.sendMessage({ type: 'GHL_CHECK_DUPLICATE', locationId: locationId, phones: phones, email: email }, function (resp) {
        cb(resp || { exists: false });
    });
}

function ghlSendLead_website(locationId, item, opts, cb) {
    chrome.runtime.sendMessage({ type: 'GHL_SEND_LEAD', locationId: locationId, item: item, opts: opts || {} }, function (resp) {
        cb(resp || { success: false, error: 'No response from background worker' });
    });
}

// ============================================================================
// GoHighLevel Confirmation Modal (website overlay)
// ============================================================================
function showCrmConfirmation_website(item, onConfirm) {
    ...
    modal.innerHTML =
        ...
        '<textarea id="gmes-confirm-note" style="width:100%;min-height:60px;padding:6px 10px;border:1.5px solid #e2e5eb;border-radius:7px;font-family:inherit;font-size:12.5px;background:#fff;color:#202124;resize:vertical;">' + esc(item.note || '') + '</textarea></td></tr>' +
        '</table></div>' +
    ...
    function populateForLocation(locationId, defaults) {
        ...
        ghlFetchTags_website(locationId, false, function (tags) {
        ...
        ghlFetchUsers_website(locationId, false, function (users) {
        ...
        ghlFetchPipelines_website(locationId, false, function (pipelines) {
    ...
    chrome.storage.local.get([
        ...
        var activeLoc = data.gmes_ghl_default_location || '';
        var defaults = {
        ...
        ghlFetchLocations_website(false, function (locs) {
...
(function () {
    ...
    function createOverlay(data) {
        ...
        // Check GHL connection and wire Send to GHL button
        ...
        if (crmStatusDiv && crmBtn) {
            ghlIsConnected_website((connected) => {
                if (!connected) {
                    ...
                    crmStatusDiv.innerHTML = '\uD83D\uDD12 Connect GoHighLevel to send leads. <a href="#" id="gmes-crm-login-link-w" style="color:#1a73e8;font-weight:700;text-decoration:underline;">Connect \u2192</a>';
                ...
                // Build the lead item from the current overlay state.
                function buildCrmItem() {
                ...
                function showSendButton() {
                    ...
                    crmBtn.addEventListener('click', () => {
                        const crmItem = buildCrmItem();
                        // Show confirmation modal — opts come back from the modal selections
                        showCrmConfirmation_website(crmItem, (opts) => {
                            crmBtn.disabled = true;
                            crmBtn.textContent = 'Sending to GHL…';
                            ghlSendLead_website(opts.locationId, crmItem, opts, (result) => {
                                if (result && result.success) {
                                    crmBtn.style.display = 'none';
                                    crmStatusDiv.textContent = result.duplicate ? '✓ Already in GHL' : '✓ Sent to GHL';
                ...
                chrome.storage.local.get(['gmes_ghl_default_location'], (cfg) => {
                    const defaultLoc = cfg.gmes_ghl_default_location || '';
                    const primaryPhone = data.phoneEntries && data.phoneEntries.length ? data.phoneEntries[0].number : '';
                    const checkPhones = primaryPhone ? [{ number: primaryPhone }] : [];
                    const primaryEmail = data.emails && data.emails.length ? data.emails[0] : '';

                    if (!defaultLoc || (!checkPhones.length && !primaryEmail)) {
                        showSendButton();
                        return;
                    }
                    ghlCheckDuplicate_website(defaultLoc, checkPhones, primaryEmail, (res) => {
                        if (res && res.exists) {
    ...
    chrome.storage.local.get(['gmes_extension_enabled'], function (data) {
        if (data.gmes_extension_enabled !== true) return;
        initWebsiteOverlay();
    });
    ...
    chrome.storage.onChanged.addListener(function (changes, area) {
        ...
        var enabled = changes.gmes_extension_enabled.newValue === true;
