(function () {
  ...
  var settingsIconGear = document.getElementById('settingsIconGear');
  var settingsIconBack = document.getElementById('settingsIconBack');

  settingsToggleBtn.addEventListener('click', function () {
    var open = settingsContent.classList.toggle('open');
    settingsToggleBtn.classList.toggle('open', open);
    document.body.classList.toggle('settings-open', open);
    settingsIconGear.style.display = open ? 'none' : 'block';
    settingsIconBack.style.display = open ? 'block' : 'none';
    settingsToggleBtn.title = open ? 'Back' : 'Settings';
  ...
  chrome.storage.local.get(['gmes_extension_enabled'], function (data) {
    // Default OFF when unset; the worker also resets this to OFF on each browser launch.
    applyMasterPower(data.gmes_extension_enabled === true);
  });
  ...
  chrome.storage.onChanged.addListener(function (changes, area) {
    ...
    if (changes.gmes_extension_enabled) {
      applyMasterPower(changes.gmes_extension_enabled.newValue === true);
    }
  });
})();
